...
CREATE TABLE IF NOT EXISTS public.tenants (
    ...
    status TEXT DEFAULT 'pending_license', -- 'pending_license', 'active', 'suspended', 'expired'
    bound_device_id TEXT, -- Cihaz Donanım Kimliği (UUID/Android ID kilidi)
    bound_device_info TEXT, -- Cihaz Modeli / Tarayıcı
    license_type TEXT DEFAULT '30_days', -- '30_days' veya 'unlimited'
    activated_at TIMESTAMPTZ, -- İlk kez uygulamada açıldığı an (süre burada başlar)
    expires_at TIMESTAMPTZ,
    grace_period_until TIMESTAMPTZ, -- 7 günlük hoşgörü ek süresi sonu
    overdue_days_deducted INT DEFAULT 0,
    notes TEXT,
    created_at TIMESTAMPTZ DEFAULT NOW(),
...
ALTER TABLE public.tenants ADD COLUMN IF NOT EXISTS license_type TEXT DEFAULT '30_days';
ALTER TABLE public.tenants ADD COLUMN IF NOT EXISTS activated_at TIMESTAMPTZ;
ALTER TABLE public.tenants ADD COLUMN IF NOT EXISTS grace_period_until TIMESTAMPTZ;
ALTER TABLE public.tenants ADD COLUMN IF NOT EXISTS overdue_days_deducted INT DEFAULT 0;
